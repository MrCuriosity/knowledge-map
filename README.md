...
- generator function, 星号函数返回一个迭代器(`iterator`), 通过调用迭代器的 next 方法(`iterator.next()`)来改变迭代器的指针，每次移动指针，迭代器指向一个包含了每一步 yield 返回值的对象(`{ value: 'someYieldValue', done: false }`)，`yield`后面的表达式可以是同步也可以是异步的，星号函数使 Promise 避免了链式调用的嵌套，优化了错误捕捉，并且可以通过`iterator.next(injectedValue)`来改变上一个 yield 的返回值，从而达到注入式影响执行逻辑的目的，这在写测试中非常实用.
- async await, 可以看做是一个自动执行每一步 iterator 的迭代器，在已知要执行一系列 Promise 并且中间不用停顿控制的情况下，是 Promise 比较好的语法糖.
...
- react
  ...
  - 有哪些优化手段
    ...
    - PureComponent 替代 Component，避免无意义的 diff(跑到 render()去, 却又 diff 不出来任何变化，此时，从入口到 render 之间的代码开销全是浪费的，如果这中间有一定的计算量，数据规模又特别大，比如日历视图渲染几千上万个格子，那么这里节约的性能是很可观的)
    - memo 包裹函数组件，同上
...
- 虚拟机与容器区别在哪
  - 虚拟机重量级，容器轻量级
  - VM在自己的OS中运行，初始化要几分钟；容器通常只有几兆，初始化几秒钟，所有容器共享操作系统
